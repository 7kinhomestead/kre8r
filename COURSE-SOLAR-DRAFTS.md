...
**The battery bank.** What it is: your savings account. The panels are the paycheck; the battery is what you live on after dark and during that cloudy week in February. Now — full honesty, because that's the deal between us — the battery on my array in that old video is a string of Duracell golf-cart batteries, 6-volt lead-acid wired in series up to 48 volts. They worked. But **this course is built around what I've actually run for the last five years — and it's kept my family powered the whole time: four LiTime 230Ah LiFePO4 batteries at 48 volts, about 12 kilowatt-hours.** I'm not selling you the shiniest thing on the shelf; I'm showing you the rig that's been quietly doing the job in my own yard, year after year. (And full disclosure for when you outgrow it: when I built my *big* new system, I went all-in on the EG4 line — that's the step-up, and it's where Rock Rich Season 2 lives.) Why LiFePO4 and not the golf-cart route? The research is lopsided: LiFePO4 runs **4,000 to 8,000-plus cycles,** it'll take a deeper discharge without complaining, and a typical off-grid home wants 10–30 kWh of storage — so a ~12kWh bank gives you roughly a day-plus of autonomy depending on how frugal you run. *Need-it-or-skip-it:* you cannot skip a battery on a true off-grid system. But you **can and should skip lead-acid** for a new build — the lithium costs more up front and saves you money and headaches across its life. A dollar saved across 6,000 cycles is worth more than a dollar earned. **Need: always lithium for a new off-grid build.**
...
That single bond — the main bonding jumper — ties your neutral to ground at one and only one point. Put in *two* and you create parallel paths that push return current onto your grounding conductors, which is exactly the thing grounding is supposed to prevent.

Here's where it gets specific to *your* box — and it settles a question that trips up every electrician who learned on grid houses: **is the panel my inverter feeds a *service* panel or a *subpanel*?** The code book answers it. An off-grid inverter is a **separately derived system** (NEC 250.30) — its own source of power, not the utility's — and the rule for one of those is a single **system bonding jumper**, made at the **source or the first disconnect, and never in more than one place.**

On my SunGold that bond is **internal**: menu item **#63** ("automatic switching of N-G connection") makes the neutral-ground bond *inside* the inverter whenever I'm running off-grid, and drops it automatically if grid or generator power ever feeds in. That inverter *is* the source — so that internal bond *is* my one and only bond.

**Now hear me on this, because it's the whole ballgame: don't assume your inverter does what mine does.** Not every all-in-one has an internal neutral-ground bond — plenty have *none* and expect you to make it yourself, and some that have one ship with it switched off. Before you wire a single thing, open *your* inverter's manual and answer one question: *does this unit bond neutral-to-ground internally, and is it on?* That one fact decides everything below — whether your feed panel is a floating-neutral subpanel or the place you make the bond yourself. Don't borrow my answer; go find yours.

And here's the part people miss: because the bond is already made upstream, **the panel my inverter feeds sits on the *load side* of it — which makes it a subpanel, not a service panel.** So I wire it like any subpanel: **float the neutral** (neutral bus isolated from the metal enclosure), **bond the ground bus** to the box, and put **no neutral-ground bond in that panel.** Landing a bonding screw there like it's a normal service panel is the classic double-bond — and code names it directly: no neutral-to-case bond on the load side of the system bonding jumper (NEC 250.142(B)).

(The flip side, so you've got the whole map: if your inverter's output *isn't* internally bonded — a true floating-neutral unit, or that #63 setting switched off — then no bond exists yet, and you make the single bond **at that first panel**, treating *it* as the bonding point. Either way the rule holds: exactly one bond, and everything downstream of it floats.)

And here's the ten-dollar way to *prove* you nailed it: with no grid feeding in, plug a cheap **outlet tester** into a receptacle. **"Correct"** means your one bond is live and doing its job. **"Open neutral"** means *zero* bonds are working — if you're counting on the inverter's internal bond, your #63 setting isn't doing what you think, so go sort that before you trust the system. Don't eyeball this; test it. **And your inverter's manual is the final word on where your bond lives.**
...
**What you'll take:** Bonding ties your metal together; grounding ties your system to the earth — and you do both *before* any power connection. The one rule to tattoo on the inside of your eyelids: **exactly one neutral-ground bond.** An off-grid inverter is a separately derived system, so that bond lives at the inverter (or the first disconnect) — which means every panel downstream is a **subpanel with a floating neutral.** One bond, never two; a $10 outlet tester reading "correct" is how you prove it.

**Your move this week:** Drive your ground rod (borrow the SDS-Max, save your marriage) and run your frame-to-frame bonding jumper. Then settle your *one* bond — and start by opening your inverter's manual to find out whether it bonds neutral-to-ground internally (on the SunGold it's item #63). **If it does:** confirm it's on for off-grid and wire the feed panel as a **subpanel** — neutral floated off the box, ground bus bonded, no N-G screw. **If it doesn't:** make that single bond at the first panel instead, treating it as the bonding point. Either way, plug an outlet tester into a receptacle and confirm it reads "correct," not "open neutral." Don't connect any power yet. We size the cables next.
