...
const TEAL     = '3ECFB2';
const TEAL_D   = '0A8C78';
const TEAL_HD  = '064E3B'; // very dark teal for header bg
const AMBER    = 'D97706';
  ...
  return new Table({
    ...
    rows: [new TableRow({
      children: [new TableCell({
        width: { size: W, type: WidthType.DXA },
        shading: { fill: TEAL_HD, type: ShadingType.CLEAR },
        margins: { top: 480, bottom: 480, left: 360, right: 360 },
        children: [
          new Paragraph({
            alignment: AlignmentType.CENTER,
            children: [new TextRun({ text: 'KRE8ΩR', font: 'Arial Black', size: 72, bold: true, color: WHITE })],
            spacing: { before: 0, after: 80 },
          }),
          new Paragraph({
            alignment: AlignmentType.CENTER,
            children: [new TextRun({ text: 'PRODUCTION RUNBOOK V2', font: 'Arial', size: 28, color: TEAL, bold: true })],
            spacing: { before: 0, after: 120 },
          }),
          new Paragraph({
            alignment: AlignmentType.CENTER,
            children: [new TextRun({ text: 'Plan it. Make it. Distribute it. MirrΩr it.', font: 'Arial', size: 22, color: WHITE, italics: true })],
...
function phaseHeader(phase, title, philosophy) {
  return [
    ...
    new Table({
      ...
      rows: [new TableRow({
        children: [new TableCell({
          ...
          children: [
            new Paragraph({
              children: [new TextRun({ text: phase.toUpperCase(), font: 'Arial', size: 18, color: TEAL, bold: true, characterSpacing: 120 })],
              spacing: { before: 0, after: 60 },
            }),
            new Paragraph({
              children: [new TextRun({ text: title, font: 'Arial Black', size: 36, bold: true, color: WHITE })],
              spacing: { before: 0, after: 0 },
            }),
          ],
        })]
    ...
    new Paragraph({
      children: [new TextRun({ text: philosophy, font: 'Arial', size: 22, color: DARK, italics: true })],
...
function toolSection(icon, name, url, whatItDoes, whatItFrees, bullets, ifThens, note) {
  ...
  parts.push(
    new Paragraph({
      ...
      children: [new Bookmark({ id: name.replace(/[^a-zA-Z]/g, ''), children: [
        new TextRun({ text: icon + '  ' + name, font: 'Arial', size: 32, bold: true, color: TEAL_D }),
      ]})],
      spacing: { before: 280, after: 80 },
    }),
    new Paragraph({
      children: [new TextRun({ text: url, font: 'Courier New', size: 20, color: GRAY_BD })],
  ...
  parts.push(
    new Paragraph({
      children: [new TextRun({ text: 'WHAT IT DOES', font: 'Arial', size: 18, bold: true, color: DARK, characterSpacing: 80 })],
      spacing: { before: 0, after: 80 },
    }),
    new Paragraph({
      children: [new TextRun({ text: whatItDoes, font: 'Arial', size: 22, color: DARK })],
  ...
  parts.push(
    new Table({
      ...
      rows: [new TableRow({ children: [new TableCell({
        ...
        borders: { left: { style: BorderStyle.SINGLE, size: 12, color: TEAL } },
        children: [
          new Paragraph({
            children: [new TextRun({ text: 'WHAT IT FREES YOU TO DO', font: 'Arial', size: 17, bold: true, color: TEAL_D, characterSpacing: 60 })],
            spacing: { before: 0, after: 60 },
          }),
          new Paragraph({
            children: [new TextRun({ text: whatItFrees, font: 'Arial', size: 22, color: DARK, italics: true })],
            spacing: { before: 0, after: 0 },
          }),
        ],
      })]})],
  ...
  // Bullets (key buttons, screens, requirements)
    ...
    bullets.forEach(b => {
      parts.push(new Paragraph({
        ...
        children: [typeof b === 'string'
          ? new TextRun({ text: b, font: 'Arial', size: 21, color: DARK })
          : b],
  ...
  if (ifThens && ifThens.length) {
    parts.push(...sp(1));
    const ifThenRows = ifThens.map(([cond, action]) =>
      new TableRow({ children: [
        new TableCell({
          width: { size: 3800, type: WidthType.DXA },
          shading: { fill: AMBER_BG, type: ShadingType.CLEAR },
          margins: { top: 80, bottom: 80, left: 120, right: 120 },
          borders: { right: { style: BorderStyle.SINGLE, size: 2, color: AMBER_BD } },
        ...
        new TableCell({
          ...
          shading: { fill: AMBER_BG, type: ShadingType.CLEAR },
          margins: { top: 80, bottom: 80, left: 120, right: 120 },
          children: [new Paragraph({ children: [new TextRun({ text: action, font: 'Arial', size: 20, color: DARK })] })],
        }),
      ]})
    );
    ...
    const headerRow = new TableRow({ tableHeader: true, children: [
      new TableCell({
        width: { size: 3800, type: WidthType.DXA },
        shading: { fill: AMBER_BD, type: ShadingType.CLEAR },
        margins: { top: 80, bottom: 80, left: 120, right: 120 },
        children: [new Paragraph({ children: [new TextRun({ text: 'IF…', font: 'Arial', size: 19, bold: true, color: WHITE })] })],
      }),
      new TableCell({
        width: { size: 5560, type: WidthType.DXA },
        shading: { fill: AMBER_BD, type: ShadingType.CLEAR },
        margins: { top: 80, bottom: 80, left: 120, right: 120 },
        children: [new Paragraph({ children: [new TextRun({ text: 'THEN…', font: 'Arial', size: 19, bold: true, color: WHITE })] })],
      }),
    ]});
  ...
  if (note) {
    parts.push(...sp(1));
    parts.push(new Paragraph({
      children: [new TextRun({ text: 'ⓘ  ' + note, font: 'Arial', size: 20, color: GRAY_BD, italics: true })],
  ...
  parts.push(
    ...
    new Table({
      ...
      rows: [new TableRow({ children: [new TableCell({
        ...
        borders: {
          top: { style: BorderStyle.DASHED, size: 2, color: GRAY_BD },
          bottom: { style: BorderStyle.DASHED, size: 2, color: GRAY_BD },
          left: { style: BorderStyle.DASHED, size: 2, color: GRAY_BD },
          right: { style: BorderStyle.DASHED, size: 2, color: GRAY_BD },
        },
        children: [new Paragraph({
          alignment: AlignmentType.CENTER,
          children: [new TextRun({ text: '[SCREENSHOT: ' + name + ']', font: 'Arial', size: 20, color: GRAY_BD, italics: true })],
        })],
      })]})],
    }),
    ...sp(1),
...
function colorBox(bgColor, borderColor, labelText, lines) {
  return new Table({
    width: { size: W, type: WidthType.DXA },
    columnWidths: [W],
    borders: { top: { style: BorderStyle.NONE }, bottom: { style: BorderStyle.NONE },
    ...
    rows: [new TableRow({ children: [new TableCell({
      ...
      children: [
        new Paragraph({
          children: [new TextRun({ text: labelText, font: 'Arial', size: 19, bold: true, color: borderColor, characterSpacing: 80 })],
          spacing: { before: 0, after: 100 },
        }),
        ...lines.map(line => new Paragraph({
          numbering: { reference: 'bullets', level: 0 },
          children: [new TextRun({ text: line, font: 'Arial', size: 21, color: DARK })],
          spacing: { before: 40, after: 40 },
        })),
      ],
    })]})],
...
function codeBox(lines) {
  return new Table({
    ...
    rows: [new TableRow({ children: [new TableCell({
      width: { size: W, type: WidthType.DXA },
      shading: { fill: '1E2A26', type: ShadingType.CLEAR },
      margins: { top: 160, bottom: 160, left: 220, right: 220 },
      children: lines.map(line => new Paragraph({
        children: [new TextRun({ text: line, font: 'Courier New', size: 20, color: TEAL })],
...
const children = [
  ...
  new Paragraph({
    alignment: AlignmentType.CENTER,
    children: [new TextRun({ text: new Date().toLocaleDateString('en-US', { year:'numeric', month:'long' }), font: 'Arial', size: 22, color: GRAY_BD })],
    spacing: { after: 400 },
  }),
  ...
  new Paragraph({
    ...
    children: [new Bookmark({ id: 'manifesto', children: [new TextRun({ text: 'The Manifesto', font: 'Arial', bold: true })] })],
    ...
    rows: [new TableRow({ children: [new TableCell({
      ...
      children: [new Paragraph({
        ...
        children: [new TextRun({
          ...
          font: 'Arial', size: 24, color: WHITE, italics: true,
        })],
      })],
    })]})],
  }),
  ...
  new Paragraph({
    heading: HeadingLevel.HEADING_1,
    children: [new Bookmark({ id: 'philosophy', children: [new TextRun({ text: 'Tool Philosophy', font: 'Arial', bold: true })] })],
    spacing: { before: 240, after: 120 },
  }),
  new Paragraph({
    children: [new TextRun({ text: 'Every tool in Kre8Ωr has two parts:', font: 'Arial', size: 22, color: DARK, bold: true })],
    spacing: { after: 120 },
  }),
  new Paragraph({
    numbering: { reference: 'numbers', level: 0 },
    children: [new TextRun({ text: 'What it does technically', font: 'Arial', size: 22, color: DARK })],
    spacing: { before: 40, after: 40 },
  }),
  new Paragraph({
    numbering: { reference: 'numbers', level: 0 },
    children: [new TextRun({ text: 'What it frees you to do creatively', font: 'Arial', size: 22, color: DARK })],
    spacing: { before: 40, after: 120 },
  }),
  new Paragraph({
    children: [new TextRun({ text: 'The technical description tells you what buttons to push. The creative description is the reason the tool exists. Read both.', font: 'Arial', size: 22, color: DARK, italics: true })],
    spacing: { after: 200 },
  }),
  tealRule(),
  ...
  ...toolSection(
    '🗺️', 'PipΩr', '/pipr.html',
    'Story structure builder. Takes your concept and maps it to a proven narrative framework — Save the Cat, Story Circle, VSL Arc, or Free Form. Generates a beat map: the skeleton of your video broken into named beats with shot types, emotional functions, and duration targets. This beat map drives WritΩr, DirectΩr, EditΩr, and ComposΩr downstream.',
  ...
  ...toolSection(
    '✍️', 'WritΩr', '/writr.html',
    'Script generator in your actual voice. Reads your beat map, Id8Ωr research brief, creator-profile.json voice analysis, and approved style. Simultaneously generates three versions: Full Script, Bullet Points, and Hybrid. Voice blend slider adjusts the ratio between your raw voice and a more polished edit. Beat cards show the emotional function of each section so you know what you\'re trying to accomplish before you write a word.',
    'The blank page is where creativity goes to die. WritΩr eliminates the blank page. What you get back is a starting point written in your voice — something to react to, improve, make your own. Your best ad-libs happen when you\'re NOT worried about what comes next. Knowing the structure, you can be fully present in the performance.',
    [
      ...
      'Content intelligence from MirrΩr is automatically injected into the top 3 Id8Ωr insights block',
    ...
    [
      ...
      ['WritΩr injecting wrong research', 'Verify Id8Ωr brief was sent before project was opened in WritΩr'],
    ],
  ...
  ...toolSection(
    ...
    'Converts your PipΩr beat map into a production shot list. Each beat becomes a set of shots — talking head, b-roll suggestions, cutaways — based on your story structure and entry point. Generates a professional crew brief PDF for your team. Creates an offline ShootDay package (QR code to Cari\'s phone) for day-of reference without needing WiFi.',
    'On shoot day your only job is to be present and perform. DirectΩr already handled the planning. You\'re free to follow the energy of the moment, take the unexpected shot, go off-script — because the plan is already solid enough to improvise against.',
    [
      ...
      'Shot list organized by beat with shot type, description, and coverage notes',
    ],
    [
  ...
  ...toolSection(
    ...
    'Mobile-first shoot day tracker. Shows beat cards with shot types, talking head prompts, reality notes, and coverage status. Tracks which shots are covered in real time. Offline-capable — loads on Cari\'s phone via QR code without needing the Kre8Ωr server to be running.',
    'You stay in creative/performance mode. The machine tracks whether you got the shot. When something unexpected happens — when Cari sees a moment you didn\'t plan for — ShootDay remembers the plan so you don\'t have to.',
    [
    ...
    [
      ['ShootDay not loading on phone', 'Must be on same WiFi network as Kre8Ωr server, or load while connected and go offline'],
  ...
  ...toolSection(
    ...
    'The distribution happens while you\'re already on to the next creative thing. AutomatΩr is the final step where the machine actually does the sending — not just the writing. You write the content, review it, approve it, and the machine delivers it.',
    [
  ...
  ...toolSection(
    ...
    [
      'Stellar classification: O (blue, viral 10x+), B (blue-white, major hit 5-10x), A (white, strong 2-5x), G (yellow, channel average), M (red, low performer)',
      'Node position: longitude = content cluster (theme), altitude = performance above/below avg',
    ...
    [
      ['Universe shows 265 projects in pipeline', 'Pipeline filter bug fixed — YouTube imports now excluded from all tool dropdowns'],
      ...
      ['Cluster regenerates differently each run', 'Claude is non-deterministic — save good clusters to kv_store via Regenerate'],
    ],
  ...
  new Paragraph({
    heading: HeadingLevel.HEADING_1,
    children: [new Bookmark({ id: 'filepaths', children: [new TextRun({ text: 'File Paths & Where Everything Lives', font: 'Arial', bold: true })] })],
    spacing: { before: 0, after: 200 },
  }),
  codeBox([
  ...
  ...sp(1),
  new Paragraph({
    children: [new TextRun({ text: 'Storage Rules', font: 'Arial', size: 22, bold: true, color: DARK })],
    spacing: { before: 0, after: 100 },
  }),
  new Paragraph({ numbering: { reference: 'bullets', level: 0 }, spacing: { before: 40, after: 40 },
    children: [new TextRun({ text: 'D:\\ (Big Ol\' Storage Drive) — all footage, proxies, and VaultΩr paths point here', font: 'Arial', size: 21 })] }),
  new Paragraph({ numbering: { reference: 'bullets', level: 0 }, spacing: { before: 40, after: 40 },
  ...
  new Paragraph({ numbering: { reference: 'bullets', level: 0 }, spacing: { before: 40, after: 40 },
    children: [new TextRun({ text: 'H:\\ (camera SSD) — Blackmagic BRAW files land here after shoot, then moved to D:\\', font: 'Arial', size: 21 })] }),
  ...
  new Paragraph({
    heading: HeadingLevel.HEADING_1,
    children: [new Bookmark({ id: 'davinci', children: [new TextRun({ text: 'DaVinci Integration', font: 'Arial', bold: true })] })],
    spacing: { before: 240, after: 160 },
  }),
  new Paragraph({ children: [new TextRun({ text: 'DaVinci Resolve Studio 20.3.2.9 must be RUNNING before any DaVinci API calls. Kre8Ωr communicates via Python scripting API on port 9237, Local mode, Windows only.', font: 'Arial', size: 22, color: DARK })], spacing: { after: 160 } }),
  new Paragraph({ children: [new TextRun({ text: 'TRIGGER 1 — BRAW Proxy (VaultΩr)', font: 'Arial', size: 22, bold: true, color: TEAL_D })], spacing: { before: 0, after: 80 } }),
  new Paragraph({ numbering: { reference: 'numbers', level: 0 }, spacing: { before: 40, after: 40 },
    children: [new TextRun({ text: 'Open DaVinci Resolve', font: 'Arial', size: 21 })] }),
  new Paragraph({ numbering: { reference: 'numbers', level: 0 }, spacing: { before: 40, after: 40 },
    children: [new TextRun({ text: 'Drop .braw file in D:\\kre8r\\intake — VaultΩr watcher auto-detects', font: 'Arial', size: 21 })] }),
  new Paragraph({ numbering: { reference: 'numbers', level: 0 }, spacing: { before: 40, after: 40 },
    children: [new TextRun({ text: 'VaultΩr calls DaVinci Python API → proxy generated → linked in database', font: 'Arial', size: 21 })] }),
  new Paragraph({ numbering: { reference: 'numbers', level: 0 }, spacing: { before: 40, after: 40 },
    children: [new TextRun({ text: 'Proxy naming: original.braw → original_proxy.mp4 (auto-linked via findBrawByBasename)', font: 'Arial', size: 21 })] }),
  ...sp(1),
  new Paragraph({ children: [new TextRun({ text: 'TRIGGER 2 — Audio Timeline (ComposΩr)', font: 'Arial', size: 22, bold: true, color: TEAL_D })], spacing: { before: 0, after: 80 } }),
  new Paragraph({ numbering: { reference: 'numbers', level: 0 }, spacing: { before: 40, after: 40 },
    children: [new TextRun({ text: 'Open DaVinci Resolve with your project loaded', font: 'Arial', size: 21 })] }),
  new Paragraph({ numbering: { reference: 'numbers', level: 0 }, spacing: { before: 40, after: 40 },
    children: [new TextRun({ text: 'In ComposΩr, select tracks for each scene and click "Push to DaVinci →"', font: 'Arial', size: 21 })] }),
  new Paragraph({ numbering: { reference: 'numbers', level: 0 }, spacing: { before: 40, after: 40 },
    children: [new TextRun({ text: 'Creates 04_AUDIO timeline with selected music tracks at scene positions at -6dB', font: 'Arial', size: 21 })] }),
  ...sp(1),
  colorBox(RED_BG, RED_BD, 'WHAT KRES8R DOES NOT DO IN DAVINCI', [
  ...
  new Paragraph({
    heading: HeadingLevel.HEADING_1,
    children: [new Bookmark({ id: 'mistakes', children: [new TextRun({ text: 'Common Mistakes', font: 'Arial', bold: true })] })],
    spacing: { before: 0, after: 200 },
  }),
  colorBox(RED_BG, RED_BD, '⚠  DO NOT DO THESE THINGS', [
    ...
    'Clicking "Send to PipΩr" before the Vision Brief is generated in Id8Ωr — only the brief is saved, not raw research',
    'Refreshing the page during WritΩr generation — click Retry instead',
    'Editing the .db file directly with sqlite3 CLI while the server is running — all edits go through the live API',
  ...
  new Paragraph({
    heading: HeadingLevel.HEADING_1,
    children: [new Bookmark({ id: 'ifthen', children: [new TextRun({ text: 'If / Then Decision Guide', font: 'Arial', bold: true })] })],
  ...
  new Table({
    ...
    rows: [
      new TableRow({ tableHeader: true, children: [
        new TableCell({ width: { size: 4200, type: WidthType.DXA }, shading: { fill: AMBER_BD, type: ShadingType.CLEAR }, margins: { top: 100, bottom: 100, left: 140, right: 140 },
          children: [new Paragraph({ children: [new TextRun({ text: 'IF THIS HAPPENS…', font: 'Arial', size: 19, bold: true, color: WHITE })] })] }),
        new TableCell({ width: { size: 5160, type: WidthType.DXA }, shading: { fill: AMBER_BD, type: ShadingType.CLEAR }, margins: { top: 100, bottom: 100, left: 140, right: 140 },
          children: [new Paragraph({ children: [new TextRun({ text: 'DO THIS', font: 'Arial', size: 19, bold: true, color: WHITE })] })] }),
      ]}),
      ...[
      ...
      ].map(([cond, action]) => new TableRow({ children: [
        new TableCell({ width: { size: 4200, type: WidthType.DXA }, shading: { fill: AMBER_BG, type: ShadingType.CLEAR }, margins: { top: 80, bottom: 80, left: 140, right: 140 },
          borders: { right: { style: BorderStyle.SINGLE, size: 2, color: AMBER_BD } },
          children: [new Paragraph({ children: [new TextRun({ text: cond, font: 'Arial', size: 20, bold: true, color: AMBER })] })] }),
        new TableCell({ width: { size: 5160, type: WidthType.DXA }, shading: { fill: AMBER_BG, type: ShadingType.CLEAR }, margins: { top: 80, bottom: 80, left: 140, right: 140 },
          children: [new Paragraph({ children: [new TextRun({ text: action, font: 'Arial', size: 20, color: DARK })] })] }),
      ]})),
    ],
  }),
  ...
  new Table({
    ...
    rows: [new TableRow({ children: [new TableCell({
      ...
      children: [
        new Paragraph({
          alignment: AlignmentType.CENTER,
          children: [new TextRun({ text: 'The tool is the support structure.', font: 'Arial Black', size: 36, color: WHITE })],
          spacing: { after: 120 },
        }),
        new Paragraph({
          alignment: AlignmentType.CENTER,
          children: [new TextRun({ text: 'You are the art.', font: 'Arial Black', size: 52, color: TEAL })],
          spacing: { after: 200 },
        }),
        new Paragraph({
          alignment: AlignmentType.CENTER,
          children: [new TextRun({ text: 'Kre8Ωr\'s job is to make sure the support structure never gets in the way of the art.', font: 'Arial', size: 24, color: WHITE, italics: true })],
          spacing: { after: 0 },
        }),
      ],
    })]})],
  ...
  new Paragraph({
    alignment: AlignmentType.CENTER,
    children: [new TextRun({ text: 'SINE RESISTENTIA', font: 'Arial', size: 19, color: GRAY_BD, characterSpacing: 200 })],
  }),
];
  ...
  styles: {
    ...
    paragraphStyles: [
      { id: 'Heading1', name: 'Heading 1', basedOn: 'Normal', next: 'Normal', quickFormat: true,
        run: { size: 40, bold: true, font: 'Arial Black', color: DARK },
        paragraph: { spacing: { before: 360, after: 200 }, outlineLevel: 0 } },
      { id: 'Heading2', name: 'Heading 2', basedOn: 'Normal', next: 'Normal', quickFormat: true,
        run: { size: 30, bold: true, font: 'Arial Black', color: TEAL_D },
  ...
  numbering: {
    config: [
      { reference: 'bullets', levels: [{ level: 0, format: LevelFormat.BULLET, text: '•',
        ...
        style: { paragraph: { indent: { left: 480, hanging: 240 } }, run: { font: 'Arial', size: 22 } } }] },
  ...
  sections: [{
    ...
    headers: {
      default: new Header({
        children: [new Paragraph({
          children: [
            new TextRun({ text: 'KRE8ΩR PRODUCTION RUNBOOK V2', font: 'Arial', size: 16, color: GRAY_BD, bold: true }),
            new TextRun({ text: '\t', font: 'Arial', size: 16 }),
            new TextRun({ text: 'SINE RESISTENTIA', font: 'Arial', size: 16, color: GRAY_BD, italics: true }),
          ],
          tabStops: [{ type: 'right', position: 8280 }],
          border: { bottom: { style: BorderStyle.SINGLE, size: 2, color: GRAY_BD, space: 1 } },
        })],
      }),
    },
    footers: {
      default: new Footer({
        children: [new Paragraph({
          children: [
            new TextRun({ text: 'kre8r.app — Plan it. Make it. Distribute it. MirrΩr it.', font: 'Arial', size: 16, color: GRAY_BD }),
            new TextRun({ text: '\t', font: 'Arial', size: 16 }),
            new TextRun({ text: 'Page ', font: 'Arial', size: 16, color: GRAY_BD }),
            new TextRun({ children: [PageNumber.CURRENT], font: 'Arial', size: 16, color: GRAY_BD }),
          ],
          tabStops: [{ type: 'right', position: 8280 }],
          border: { top: { style: BorderStyle.SINGLE, size: 2, color: GRAY_BD, space: 1 } },
        })],
      }),
    },
    children,
